...
.PHONY: help install dev stop test test-unit test-integration test-load evaluate build clean check-files pre-delivery
...
check-files: ## Verifica archivos mínimos esperados para E3
	...
	@test -d tests/unit || (echo "Falta tests/unit" && exit 1)
	...
	@echo "Archivos mínimos de E3 presentes"

pre-delivery: check-files test build ## Ejecuta validaciones principales antes de entrega
	@echo "Proyecto validado para entrega E3"